...
class CRUDPares
{
    ...
    public function obtener_articulos(){
        if($this->pares!=null)
            $this->sql="select ar.* from articulos ar WHERE ar.id in( SELECT ar.id from articulos ar inner JOIN revisiones re on ar.id=re.articulo WHERE re.par='{$this->pares->getNombre()}' or re.par2='{$this->pares->getNombre()}'  or re.par3='{$this->pares->getNombre()}' )";
