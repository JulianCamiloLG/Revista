...
include_once ('../models/Revision.php');
include_once ('../models/CRUDRevision.php');
...
class controladora 
{
	...
	public function articulos_par(){
		session_start();
		if($_SESSION['tipo']=="par")

		{
			$crud=new CRUDPares($_SESSION["usuario"]);
	...
	public function update_pares($pares){
		extract($pares);
		echo json_encode($pares);
		$revision=new Revision($id,$par,"","","",$par2,$par3,"","","","");
		$crud=new CRUDRevision($revision);
		$crud->modificar_pares();
	}

	public function calificar($datos){
		session_start();
		extract($datos);
		$revision=new Revision($id,"","",$comentarios,"","","","","","","","");
		$crud=new CRUDRevision($revision);
		$crud->calificar($_SESSION["usuario"],$nota);
	}
}
...
elseif($funcion=="updatepares"){
	$obj->update_pares($_REQUEST["json"]);
}
elseif($funcion=="calificar"){
	$datos=$_REQUEST["json"];
	$obj->calificar($datos);
}
